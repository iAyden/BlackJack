...
def main():
    ...
    for i in player_hand:
        if i.value == 11 and sum_player>21:
            sum_player = sum([c[0] for c in player_cards])-10
    for i in dealer_hand:
        if i.value == 11 and sum_dealer>21:
            sum_dealer = sum([c[0] for c in dealer_cards])-10
    while running:
        screen.fill(GREEN)
